...
export function CreateProjectForm({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.name.trim()) {
            setStatus(prev => ({ ...prev, error: "Название проекта не может быть пустым" }));
            return;
        }
        ...
        try {
            ...
            if (formData.selectedUsers.length > 0 && project?.id && spaceData.memberRoleId) {
                await Promise.all(
        ...
        <form onSubmit={handleSubmit} className={cn("grid items-start gap-6", className)}>
            ...
            <div className="flex gap-4">
                <IconPicker
                    ...
                    onPick={(selectedIcon) => setFormData(prev => ({ ...prev, icon: selectedIcon }))}
                    asChild
                />
            ...
            <div className="grid gap-3">
                ...
                {status.isLoading ? (
                    ...
                    <div className="text-sm text-muted-foreground">Нет доступных пользователей</div>
                ) : (
            ...
            <Button
                type="submit"
                disabled={!formData.name.trim() || status.isCreating || status.isLoading}
                className="w-full"
            >
